...
class XVLA(PreTrainedModel):
    ...
    def forward_vlm(
        ...
        # TODO (dogganon): torch.compile only supports asserts
        # if not torch.any(flat_mask):
        #     raise ValueError("At least one image view must be valid per batch.")        
        assert torch.any(flat_mask), "At least one image view must be valid per batch."

        # TODO (dogganon): torch.compile requires static shapes for full graph
        ...
        image_features[~flat_mask] = 0
        N, D = image_features.shape[1:]
