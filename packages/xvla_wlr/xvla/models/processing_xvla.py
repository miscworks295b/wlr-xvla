...
class XVLAProcessor(ProcessorMixin):
    ...
    def encode_language(self, language_instruction: Union[str, List[str]]) -> Dict[str, torch.Tensor]:
        ...
        language_instruction = numpy.asarray(language_instruction).reshape(-1).tolist()
    ...
    def process_image(
        ...
        **kwargs,
        ...
        images_, image_batch_shapes = einops.pack(
            [images],
            "* channel height width",
        )
        images_ = self.image_processor(images_, return_tensors="pt", **kwargs)["pixel_values"]
        [images_] = einops.unpack(images_, image_batch_shapes, "* channel height width")
        return images_
